...
def analyser(partie, nom_equipe: str, index_trimestre: int = -1) -> dict:
    ...
    liq = ratios.get("liquidite_generale")
    if liq is not None and liq < 1.0:
        diagnostics.append(Diagnostic(
            "liquidite_critique", "Finance", A, "Liquidité insuffisante",
            f"Ratio de liquidité générale de {liq:.2f} : vos actifs à court terme "
            f"ne couvrent pas vos dettes à court terme.",
            "En dessous de 1, l'entreprise ne peut pas honorer ses échéances avec "
            "ses seuls actifs courants. Reconstituez de la trésorerie ou "
            "rééchelonnez vos engagements."))
    elif liq is not None and liq > 3.5:
        diagnostics.append(Diagnostic(
            "liquidite_dormante", "Finance", N, "Liquidité très confortable",
            f"Ratio de liquidité générale de {liq:.2f}.",
            "Une trésorerie abondante est sécurisante, mais du capital inemployé "
            "ne produit rien : envisagez d'investir en capacité, en R&D ou en "
            "développement du réseau."))

    fr = ratios.get("fonds_roulement")
    fr_rev = ratios.get("fonds_roulement_sur_revenus")
    if fr is not None and fr < 0:
        diagnostics.append(Diagnostic(
            "fdr_negatif", "Finance", A, "Fonds de roulement négatif",
            f"Fonds de roulement de {_n(fr)} HTG.",
            "Vous financez des actifs durables avec des ressources à court terme : "
            "c'est la configuration classique qui précède une crise de trésorerie."))
    elif fr_rev is not None and fr_rev > 0.9:
        diagnostics.append(Diagnostic(
            "fdr_lourd", "Finance", N, "Fonds de roulement très élevé",
            f"Il représente {_pct(fr_rev)} de vos revenus du trimestre.",
            "Vérifiez la composition : des stocks et des créances trop lourds "
            "immobilisent du capital sans le rémunérer."))

    dr = ratios.get("delai_recouvrement_jours")
    if dr is not None and dr > 40:
        diagnostics.append(Diagnostic(
            "recouvrement", "Finance", A, "Délai de recouvrement long",
            f"Environ {dr:.0f} jours pour encaisser vos ventes.",
            "Vendre n'est pas encaisser : plus le délai s'allonge, plus vous "
            "financez vos clients sur votre propre trésorerie."))
    ...
    if rh:
        if rh["moral"] < 40:
            diagnostics.append(Diagnostic(
                "moral_bas", "Ressources humaines", A, "Moral des équipes dégradé",
                f"Indice de moral à {rh['moral']:.0f}/100, salaire offert à "
                f"{_pct(rh['ratio_salaire'] - 1)} du niveau du marché"
                + (f", {rh['departs_naturels']} départ(s) volontaire(s) ce trimestre"
                   if rh["departs_naturels"] else "") + ".",
                "Un moral bas réduit la productivité, la qualité produite et "
                "provoque des départs. Salaire, avantages sociaux et formation "
                "sont les leviers disponibles."))
        elif rh["moral"] > 75:
            diagnostics.append(Diagnostic(
                "moral_haut", "Ressources humaines", P, "Équipes très motivées",
                f"Indice de moral à {rh['moral']:.0f}/100, productivité à "
                f"{rh['productivite']:.2f}.",
                "Cet investissement humain se traduit directement en capacité de "
                "production et en qualité : c'est un avantage durable."))
        if rh["competence"] > 0.08:
            diagnostics.append(Diagnostic(
                "competence", "Ressources humaines", P, "La formation porte ses fruits",
                f"Gain de compétence de {_pct(rh['competence'])} sur la productivité.",
                "Compétence acquise et cumulative : elle réduit le besoin "
                "d'embauches pour un même volume de production."))
        if rh["departs_voulus"] > 0:
            diagnostics.append(Diagnostic(
                "licenciements", "Ressources humaines", A, "Réduction d'effectif",
                f"{rh['departs_voulus']} départ(s) décidé(s), avec indemnités.",
                "Les licenciements coûtent immédiatement et pèsent sur le moral "
                "des équipes restantes. Assurez-vous que la capacité de production "
                "reste suffisante."))

    # --- MARKETING ET RÉSEAU -------------------------------------------
    dec = rap.get("decisions")
    reseau = rap.get("reseau")
    if dec is not None:
        budgets = getattr(dec, "marketing", {}) or {}
        total_mkt = sum(budgets.values())
        if total_mkt > 0:
            principal = max(budgets.items(), key=lambda x: x[1])
            if principal[1] / total_mkt > 0.85 and len(budgets) >= 1:
                diagnostics.append(Diagnostic(
                    "canal_unique", "Marketing", N, "Communication concentrée sur un canal",
                    f"{_pct(principal[1] / total_mkt)} du budget sur « {principal[0]} ».",
                    "Chaque canal sature : au-delà d'un certain montant, la gourde "
                    "suivante rapporte moins. Comparer le rendement de plusieurs "
                    "canaux permet souvent d'obtenir plus avec le même budget."))
        elif rap["etat_resultats"]["revenus"] > 0:
            diagnostics.append(Diagnostic(
                "sans_marketing", "Marketing", A, "Aucun budget de communication",
                "Vous n'avez investi dans aucun canal ce trimestre.",
                "Sans notoriété, seul le prix vous distingue — une position "
                "difficile à tenir face à des concurrents qui communiquent."))
    if reseau and reseau.get("total_pdv", 0) <= 1:
        diagnostics.append(Diagnostic(
            "reseau_etroit", "Réseau", N, "Présence géographique limitée",
            f"{reseau['total_pdv']} point(s) de vente.",
            "Sans présence dans un département, vous n'y captez presque rien, "
            "quel que soit votre budget national de communication."))
...
def rediger(analyse: dict, pour_professeur: bool = False) -> str:
    ...
    for categorie in ["Décisions", "Marché", "Production", "Coûts",
                      "Ressources humaines", "Marketing", "Réseau", "Finance",
                      "Trajectoire"]:
